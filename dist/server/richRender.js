...
import { getFiletypeFromFileName, parseDiffFromFile, parsePatchFiles, processFile, } from "@pierre/diffs";
...
// ---------------------------------------------------------------------------
const SHIKI_DARK_RULE = ".shiki, .shiki span { color: var(--shiki-dark) !important; background-color: var(--shiki-dark-bg) !important; }";
...
function shikiSchemeCss(mode) {
    if (mode === "dark")
        return SHIKI_DARK_RULE;
    if (mode === "light")
        return "";
...
async function loadLangs(hl, langs) {
    await Promise.allSettled(langs.map(async (l) => hl.loadLanguage(l)));
}
function highlight(hl, code, lang, pair) {
    if (!lang)
        return null;
    try {
...
export async function renderMarkdown(part, opts = {}) {
...
export function renderTerminal(part) {
    const au = new AnsiUp();
    ...
    const body = `<div class="term-bar"><span class="term-dots" aria-hidden="true">` +
...
export async function renderCode(part, opts = {}) {
    ...
    const filename = part.title
        ...
        : hasHead
            ...
            : "";
    const langBadge = lang && lang !== "text" ? `<span class="code-lang">${escapeHtml(lang)}</span>` : "";
...
function buildFileDiffs(part) {
    const langs = new Set(BASE_LANGS);
    const diffs = [];
    if (part.patch) {
        for (const parsed of parsePatchFiles(part.patch)) {
            for (const fd of parsed.files) {
                diffs.push(fd);
                if (fd.name)
                    langs.add(getFiletypeFromFileName(fd.name));
            }
        }
        if (diffs.length === 0) {
        ...
        for (const f of part.files) {
            ...
            langs.add(lang);
            diffs.push(parseDiffFromFile({ name: f.filename, contents: f.before, lang: lang }, { name: f.filename, contents: f.after, lang: lang }));
        }
    }
    return { diffs, langs: [...langs] };
}
export async function renderDiff(part, opts = {}) {
    ...
    const rendered = await Promise.all(diffs.map((fileDiff) => preloadFileDiff({ fileDiff, options })));
    const body = rendered
        .map((r) => `<diffs-container><template shadowrootmode="open">${r.prerenderedHTML}</template></diffs-container>`)
        .join("");
