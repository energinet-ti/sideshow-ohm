...
import { feedbackView, mcpPostListRowView } from "../server/apiViews.js";
import { MCP_INSTRUCTIONS, MCP_SERVER_INFO, MCP_TOOL_DESCRIPTIONS, STDIO_MCP_INPUT_SCHEMAS, } from "../server/mcpSpec.js";
...
async function api(path, init = {}) {
    ...
    try {
        ...
        throw new Error(`sideshow server not reachable at ${API} — ask the user to start it with "sideshow serve" or "npm run dev"`);
    }
...
let sessionId = process.env.SIDESHOW_SESSION ?? null;
...
async function ensureSession(title) {
    if (sessionId)
        return sessionId;
    const session = JSON.parse(await api("/api/sessions", {
        method: "POST",
...
server.registerTool("publish_post", {
    ...
    const created = JSON.parse(await api("/api/posts", {
        method: "POST",
...
server.registerTool("update_post", {
    ...
    const updated = JSON.parse(await api(`/api/posts/${id}`, { method: "PUT", body: JSON.stringify({ surfaces, title }) }));
    return text({ ...updated, url: `${API}/p/${updated.id}` });
});
server.registerTool("list_posts", { description: MCP_TOOL_DESCRIPTIONS.listPostsStdio, inputSchema: {} }, async () => {
    if (!sessionId)
        return text([]);
...
server.registerTool("publish_surface", {
...
server.registerTool("wait_for_feedback", {
    ...
    const result = JSON.parse(await api(`/api/comments?session=${session}&author=user&wait=${wait}`));
...
server.registerTool("reply_to_user", {
    ...
    const created = JSON.parse(await api("/api/comments", {
        method: "POST",
        body: JSON.stringify({ surface: postId ?? surfaceId, text: message, author: AGENT }),
    }));
    return text(created);
});
server.registerTool("list_surfaces", { description: MCP_TOOL_DESCRIPTIONS.listSurfacesStdio, inputSchema: {} }, async () => {
    if (!sessionId)
        return text([]);
...
server.registerTool("upload_asset", {
    ...
    const session = await ensureSession();
    const created = JSON.parse(await api("/api/assets", {
        method: "POST",
...
server.registerTool("add_surface", {
    ...
    const updated = JSON.parse(await api(`/api/posts/${postId}/surfaces`, {
        method: "POST",
        body: JSON.stringify({ surface, before, after }),
    }));
    return text({ ...updated, url: `${API}/p/${updated.id}` });
});
server.registerTool("edit_surface", {
    ...
    const updated = JSON.parse(await api(`/api/posts/${postId}/surfaces/${target}`, {
        method: "PATCH",
        body: JSON.stringify({ surface, content, kits }),
    }));
    return text({ ...updated, url: `${API}/p/${updated.id}` });
});
server.registerTool("remove_surface", {
    description: MCP_TOOL_DESCRIPTIONS.removeSurface,
    inputSchema: STDIO_MCP_INPUT_SCHEMAS.removeSurface,
}, async ({ postId, target }) => {
    const updated = JSON.parse(await api(`/api/posts/${postId}/surfaces/${target}`, { method: "DELETE" }));
    return text({ ...updated, url: `${API}/p/${updated.id}` });
});
server.registerTool("reorder_surfaces", {
