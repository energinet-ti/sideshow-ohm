...
import { migrateLegacyDataDir } from "./migrateDataDir.js";
import { SqlStore } from "./sqlStore.js";
import { createSqliteStorage, migrateJsonToSqlite } from "./sqliteStorage.js";
...
const defaultThemeId = process.env.SIDESHOW_DEFAULT_THEME &&
...
if (process.env.SIDESHOW_STORE === "json") {
    ...
    console.log(`sideshow store: SQLite at ${dbPath} (SIDESHOW_STORE=json for the legacy JSON store)`);
}
const app = createApp({
    store,
    viewerHtml,
    ...
    version: process.env.SIDESHOW_VERSION ?? JSON.parse(pkgJson).version,
