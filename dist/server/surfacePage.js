import { kitAssets } from "./kits.js";
import { schemeCss, themeById, tokenThemeCss, viewerThemeCss, } from "./themes.js";
...
const kitAccentCss = (mode) => schemeCss(KIT_ACCENTS_LIGHT, KIT_ACCENTS_DARK, mode);
...
const colorSchemeCss = (mode) => `:root{color-scheme:${mode ?? "light dark"}}`;
...
export function renderSandboxedPart(doc) {
    const theme = typeof doc.theme === "string" || doc.theme == null ? themeById(doc.theme) : doc.theme;
    return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
function mermaidThemeVars(p, mode) {
    const text = p.text;
    const muted = p.muted;
