export const surfaceRef = (surface, index) => ({
    id: surface.id,
    kind: surface.kind,
    index,
});
export const fullSurfaceView = (surface, index) => ({ ...surface, index });
...
export const sessionListSurfaceView = (surface, index) => surface.kind === "html" ? surfaceRef(surface, index) : fullSurfaceView(surface, index);
export const postWriteView = (post) => ({
    id: post.id,
...
export const sessionPostListRowView = (post) => {
...
export const mcpPostListRowView = (post) => ({
    id: post.id,
    sessionId: post.sessionId,
    title: post.title,
    version: post.version,
...
function capSurface(surface) {
    switch (surface.kind) {
        ...
        case "trace": {
            ...
            if ((surface.steps?.length ?? 0) > TRACE_STEP_PREVIEW_LIMIT)
                truncated = true;
            return truncated
                ? {
                    ...surface,
...
export const recentSurfacePreviewView = (surface, index) => ({
    ...capSurface(surface),
    index,
});
export const recentPostRowView = (post, session) => {
...
export const feedbackView = (comment) => ({
...
export const sessionRowView = (session, postCount) => ({
    ...session,
    postCount,
