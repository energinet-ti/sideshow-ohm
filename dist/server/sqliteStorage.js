...
process.emitWarning = function patched(warning, ...rest) {
    ...
    if (/\bSQLite is an experimental feature\b/.test(message))
        return;
    defaultEmitWarning(warning, ...rest);
};
...
export function createSqliteStorage(path = ":memory:") {
    ...
    return {
        exec(query, ...bindings) {
            ...
            const control = bindings.length === 0 &&
...
// backup.
export async function migrateJsonToSqlite(sqlite, jsonPath) {
    if ((await sqlite.getSetting("importedFrom")) != null)
        return;
    ...
    try {
        ...
        console.error(`[sideshow] could not read ${jsonPath} to migrate it into SQLite ` +
    ...
    if (snapshot.sessions.length || posts.length) {
        console.error(`[sideshow] migrated ${snapshot.sessions.length} session(s), ` +
