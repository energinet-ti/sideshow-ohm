import { z } from "zod";
import { KIT_IDS } from "./kits.js";
import { SURFACE_KIND_LIST, SURFACE_KINDS } from "./types.js";
...
const PART_KIND_ENUM = [...SURFACE_KINDS];
export const MCP_INSTRUCTIONS = "sideshow is a live visual surface the user watches in a browser. Publish posts to illustrate " +
...
const d = {
    ...
    sessionTitle: 'Session name shown in the sidebar — name the task, e.g. "Auth refactor". Honored only when this publish creates the session.',
    ...
    surfaceKits: `html surface: opt into style/behavior bundles by id (${KIT_IDS.join(" | ")}). Each injects extra CSS/JS classes (e.g. 'issues' gives .card/.tree/.badge; 'slides' gives a stepped .deck). Omit for plain html. See get_design_guide.`,
    surfaceMarkdown: "markdown surface: prose (headings, lists, tables, code, links); raw HTML is escaped",
    surfaceMermaid: "mermaid surface: diagram source (flowchart, sequence, ERD, gantt, …), rendered to SVG by the viewer",
    ...
    surfaceData: "json surface: any JSON value (object, array, string, number, boolean, null) — the viewer renders it as a collapsible tree",
    surfaceCode: "code surface: source code the viewer syntax-highlights and shows with line numbers",
    surfaceLanguage: "code surface: language id (ts, js, python, go, rust, …); omit or use 'text' for plain monospace",
    surfaceLineStart: "code surface: 1-based starting line number for an excerpt (the viewer numbers from here instead of 1)",
    surfaceTarget: "Surface id or 0-based index within the post",
};
const MCP_SURFACES_DESCRIPTION = "Ordered surfaces. html: {kind:'html', html:'<body fragment>', kits?:['issues']} — kits opt the " +
...
export const MCP_TOOL_DESCRIPTIONS = {
    ...
    updatePost: "Revise a post in place (same card, new version). Prefer this over publishing a near-duplicate. Pass the full replacement surfaces array. Returns the new surface ids (use them to target a surface for later edits without a get_post round-trip). If the result includes userFeedback, read it.",
    listPostsHttp: "List posts — pass a session id to scope, or omit for all sessions. Returns lean post rows with surfaces as `{id, kind, index}` metadata (no surface bodies).",
    listPostsStdio: "List posts in this conversation's session. Returns lean post rows with surfaces as `{id, kind, index}` metadata (no surface bodies).",
    ...
    updateSurface: "Deprecated alias of update_post — Revise a post in place (same card, new version). Prefer this over publishing a near-duplicate. Pass the full replacement surfaces array. If the result includes userFeedback, read it.",
    publishSnippet: "Publish an HTML snippet — sugar for a post with one html surface. Send a body fragment only. Returns the id, view URL, and sessionId. Pass sessionTitle on first publish. Prefer publish_post when you want a diff or multiple surfaces.",
    updateSnippet: "Revise an html snippet in place — sugar for update_post with one html surface.",
    waitForFeedback: "Block until the user comments on this session in their browser (or the timeout passes). Returns new comments since the agent last received feedback on any channel. Use timeoutSeconds 0 for a non-blocking check.",
    replyToUser: "Post a short reply under a post's comment thread. Use to acknowledge feedback or explain a revision.",
    listSurfacesHttp: "Deprecated alias of list_posts — List posts; pass a session id to scope, or omit for all sessions. Returns lean post rows with surfaces as `{id, kind, index}` metadata (no surface bodies).",
    listSurfacesStdio: "Deprecated alias of list_posts — List posts in this conversation's session. Returns lean post rows with surfaces as `{id, kind, index}` metadata (no surface bodies).",
    ...
    uploadAssetStdio: "Upload a binary asset (image, trace file, any file) and get back its id and URL. base64-encode the bytes in `data`. Then reference it: put {kind:'image', assetId} or {kind:'trace', assetId} in a post's surfaces, or embed the returned url in an html surface (<img src=\"...\">). Attached to this conversation's session.",
    getDesignGuide: "Fetch the design contract: post surfaces, html fragment rules, theme CSS variables, CDN allowlist, and the interactivity bridge. Call once per session before publishing.",
    addSurface: "Append a surface to an existing post (same card, new version). Optionally pass before/after (surface id or 0-based index) to control insert position; default is append at the end. If the result includes userFeedback, read it.",
    editSurface: "Replace or content-edit a single surface in a post (same card, new version). Pass `surface` for a full replacement, or `content` for a content-only update that preserves the surface kind and extra fields (language, cols, layout, etc.). `target` is a surface id or 0-based index. If the result includes userFeedback, read it.",
    removeSurface: "Remove a single surface from a post (same card, new version). `target` is a surface id or 0-based index. Rejects if it's the last surface (posts need at least one). If the result includes userFeedback, read it.",
    reorderSurfaces: "Reorder the surfaces in a post (same card, new version). Pass an array of surface ids or 0-based indices in the desired order; the length must match the current surface count. If the result includes userFeedback, read it.",
};
    ...
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                author: {
                    type: "string",
                    description: 'Your agent name (default "agent"; "user" is reserved and coerced to "agent")',
                },
            },
            required: ["message"],
        },
...
const mcpSurfaceSchema = z
    ...
    .describe("A surface: html {kind:'html',html}; markdown {kind:'markdown',markdown} (prose); mermaid " +
