...
import { isKnownKit, KIT_IDS } from "./kits.js";
import { isSurfaceKind } from "./types.js";
const requiredString = (name) => z.string({
    required_error: `requires string "${name}"`,
    invalid_type_error: `requires string "${name}"`,
});
const optionalLooseString = z.preprocess((v) => (typeof v === "string" ? v : undefined), z.string().optional());
const looseLayout = z.preprocess((v) => (v === "unified" || v === "split" ? v : undefined), z.enum(["unified", "split"]).optional());
const optionalLooseNumber = z.preprocess((v) => (typeof v === "number" && Number.isFinite(v) ? v : undefined), z.number().optional());
...
const filteredArray = (schema) => z.preprocess((raw) => {
...
const looseHtmlSurface = z
    ...
    return { kind: "html", html: p.html, ...(kits.length > 0 ? { kits } : {}) };
});
...
const strictJsonSurface = z
    ...
    .transform((p) => ({ kind: "json", data: p.data }));
...
async function parseSurfaceList(raw, opts = {}) {
    if (!Array.isArray(raw))
        return surfaceResult([], ["surfaces must be an array"]);
    if (opts.strict === true) {
        const results = await Promise.all(raw.map((surface, i) => parseStrictSurface(surface, i)));
        return surfaceResult(results.flatMap((r) => (r.surface ? [r.surface] : [])), results.flatMap((r) => r.errors));
    }
    const surfaces = [];
    for (const surface of raw) {
        ...
        if (!parsed.success)
            continue;
        if ((await validateSemantics(parsed.data)).length === 0)
            surfaces.push(parsed.data);
    }
    return surfaceResult(surfaces, []);
}
function surfaceResult(surfaces, errors) {
    return { surfaces, parts: surfaces, errors };
}
export const coerceSurfaces = (raw) => parseSurfaceList(raw).then((r) => r.surfaces);
export async function validateSurfaces(raw) {
...
function diffPatchHasContent(patch) {
    let files = 0;
...
function mermaidDiagramType(src) {
...
async function validateSemantics(surface) {
    ...
    if (surface.kind === "mermaid") {
        ...
        try {
            await parseMermaid(diagramType, surface.mermaid);
        }
        catch (e) {
            ...
            if (e instanceof Error && /unknown diagram type/i.test(e.message))
                return [];
...
async function parseStrictSurface(raw, index) {
    ...
    if (!schema)
        return { surface: null, errors: [`${path}: unknown surface kind`] };
    const parsed = schema.safeParse(raw);
    if (!parsed.success)
        return { surface: null, errors: formatZodErrors(parsed.error, path) };
...
function formatZodErrors(error, prefix = "surfaces") {
