import { feedbackView, mcpPostListRowView, postDetailView, postWriteView } from "./apiViews.js";
import { decodeBase64 } from "./base64.js";
import { htmlSurface, } from "./types.js";
import { HTTP_MCP_TOOLS, MCP_INSTRUCTIONS, MCP_SERVER_INFO } from "./mcpSpec.js";
import { coerceSurfaces } from "./postSurfaces.js";
...
export function registerMcp(app, deps) {
    ...
    const postResult = (result, origin) => JSON.stringify({
    ...
    async function callTool(name, args, origin) {
        switch (name) {
            ...
            case "publish_snippet": {
                ...
                const surfaces = name === "publish_snippet"
            ...
            case "update_snippet": {
                ...
                if (name === "update_snippet") {
                    ...
                    if (blocks !== undefined)
                        patch.surfaces = await coerceSurfaces(blocks);
                }
            ...
            case "reply_to_user": {
                ...
                if ("error" in result)
                    throw new Error(result.error);
                return JSON.stringify({ ...result.comment, ...(result.userFeedback && { userFeedback: result.userFeedback }) }, null, 2);
            }
            case "list_posts":
            case "list_surfaces":
            case "list_snippets": {
                const posts = await deps.store.listPosts(typeof args.session === "string" ? args.session : undefined);
            ...
            case "upload_asset": {
                ...
                const result = await deps.uploadAsset({
                    ...
                    kind: args.kind === "image" || args.kind === "trace" || args.kind === "file"
                        ? args.kind
                        : undefined,
                    session: typeof args.session === "string" ? args.session : undefined,
                });
            ...
            case "edit_surface": {
                ...
                const result = await deps.replacePostSurface(String(args.postId ?? ""), String(args.target ?? ""), {
                    surface,
            ...
            case "remove_surface": {
                const result = await deps.removePostSurface(String(args.postId ?? ""), String(args.target ?? ""));
                if ("error" in result)
                    throw new Error(result.error);
                return postResult(result, origin);
            }
            case "reorder_surfaces": {
                const result = await deps.reorderPostSurfaces(String(args.postId ?? ""), Array.isArray(args.order) ? args.order : []);
                if ("error" in result)
                    throw new Error(result.error);
                return postResult(result, origin);
            }
    ...
    app.post("/mcp", async (c) => {
        const rpc = (id, result) => c.json({ jsonrpc: "2.0", id, result });
        const rpcError = (id, code, message, status = 200) => c.json({ jsonrpc: "2.0", id, error: { code, message } }, status);
        let msg;
        try {
        ...
        if (msg.id === undefined)
            return c.body(null, 202); // notifications
        if (msg.method === "ping")
            return rpc(msg.id, {});
        if (msg.method === "tools/list")
            return rpc(msg.id, { tools: HTTP_MCP_TOOLS });
