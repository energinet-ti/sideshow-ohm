...
import { feedbackView, postDetailView, postWriteView, recentPostRowView, sessionPostListRowView, sessionRowView, } from "./apiViews.js";
import { EventBus } from "./events.js";
import { kitSummaries } from "./kits.js";
import { registerMcp } from "./mcpHttp.js";
import { escapeHtml, renderHtmlPage, renderMermaidPage, renderSandboxedPart, } from "./surfacePage.js";
import { renderCode, renderDiff, renderMarkdown, renderTerminal } from "./richRender.js";
import { DEFAULT_THEME_ID, themeById, themeOptions } from "./themes.js";
import { htmlSurface, isSandboxedSurfaceKind, MAX_ASSET_BYTES, surfacesByteLength, SURFACE_CONTENT_FIELDS, } from "./types.js";
import { validateSurfaces } from "./postSurfaces.js";
...
function assetServeHeaders(asset) {
...
const isAssetKind = (v) => v === "image" || v === "trace" || v === "file";
...
async function fetchLatestFromRegistry() {
    ...
    let notes;
    try {
        const gh = await fetch(`https://api.github.com/repos/modem-dev/sideshow/releases/tags/v${pkg.version}`, { headers: { "user-agent": "sideshow", accept: "application/vnd.github+json" } });
        if (gh.ok) {
...
function isPublicReadAllowed(path, mode) {
    if (mode === "full")
        return true;
...
export function createApp({ store, viewerHtml, guideMarkdown, setupText, agentHowtoText = setupText, authenticate, authToken, basePath, publicRead, screenshots, defaultThemeId, viewerChrome, version, upgradeCommand, fetchLatestRelease, onEvent, maxHoldConnections = DEFAULT_MAX_HOLD_CONNECTIONS, }) {
    const app = new Hono();
    const bus = new EventBus();
    const initialThemeId = defaultThemeId && themeById(defaultThemeId).id === defaultThemeId
        ? defaultThemeId
        : DEFAULT_THEME_ID;
    if (onEvent) {
        bus.subscribe((event) => {
    ...
    const acquireHold = () => {
        if (holdConnections >= maxHoldConnections)
            return false;
        holdConnections++;
        return true;
    };
    ...
    async function cachedRender(key, build) {
    ...
    const normalizeBasePath = (value) => {
        ...
        while (end > 0 && withLeading.charCodeAt(end - 1) === 47)
            end--;
        return withLeading.slice(0, end);
    };
    const requestBasePath = (request) => normalizeBasePath(typeof basePath === "function" ? basePath(request) : basePath);
    ...
    async function collectFeedback(sessionId) {
    ...
    function findSurfaceIndex(surfaces, target) {
    ...
    function applyContent(surface, content, kits) {
    ...
    async function revisePost(id, patch) {
        if (patch.surfaces) {
        ...
        if (patch.title !== undefined)
            patch.title = patch.title.slice(0, MAX_TITLE);
    ...
    async function appendPostSurface(id, surface, pos) {
        ...
        if (pos?.before !== undefined) {
            ...
            if (i < 0)
                return { error: `surface "${pos.before}" not found`, status: 404 };
            insertAt = i;
        }
        else if (pos?.after !== undefined) {
            const i = findSurfaceIndex(existing.surfaces, pos.after);
            if (i < 0)
                return { error: `surface "${pos.after}" not found`, status: 404 };
            insertAt = i + 1;
        }
    ...
    async function replacePostSurface(id, target, replacement) {
        const existing = await store.getPost(id);
        if (!existing)
            return { error: "post not found", status: 404 };
        const idx = findSurfaceIndex(existing.surfaces, target);
        if (idx < 0)
            return { error: `surface "${target}" not found`, status: 404 };
        let updated;
    ...
    async function removePostSurface(id, target) {
        const existing = await store.getPost(id);
        if (!existing)
            return { error: "post not found", status: 404 };
        const idx = findSurfaceIndex(existing.surfaces, target);
    ...
    async function reorderPostSurfaces(id, order) {
        const existing = await store.getPost(id);
        if (!existing)
            return { error: "post not found", status: 404 };
        ...
        const reordered = Array.from({ length: order.length });
        const used = new Set();
        for (const entry of order) {
            const idx = findSurfaceIndex(existing.surfaces, String(entry));
            if (idx < 0)
                return { error: `surface "${entry}" not found`, status: 404 };
            if (used.has(idx))
                return { error: `surface "${entry}" appears twice in order`, status: 400 };
            used.add(idx);
        }
    ...
    function sanitizeCommentAnchor(raw, post) {
        ...
        if (surfaceIndex < 0 || surfaceIndex >= post.surfaces.length)
            return undefined;
    ...
    async function createComment(input) {
        ...
        if (!input.surface)
            return { error: 'provide a "surface" id', status: 400 };
        ...
        const userFeedback = input.author === "user" ? undefined : await collectFeedback(comment.sessionId);
        return { comment, userFeedback };
    }
    // Long-poll: resolves as soon as a matching comment lands, or at timeout.
    async function waitForComments(q, signal) {
        ...
        const matches = (list) => q.author ? list.filter((cm) => cm.author === q.author) : list;
            ...
            await new Promise((resolve) => {
                ...
                const unsubscribe = bus.subscribe((event) => {
                    ...
                    if (q.sessionId && event.sessionId !== q.sessionId)
                        return;
                    if (q.surfaceId && event.surfaceId !== q.surfaceId)
                        return;
                    done();
                });
                const onAbort = () => done();
    ...
    const isAuthenticated = (c) => {
        ...
        if (c.req.header("authorization") === `Bearer ${authToken}`)
            return true;
        if (getCookie(c, "sideshow_key") === authToken)
            return true;
        return c.req.query("key") === authToken;
    };
    const isUnauthenticatedSessionRead = (c) => publicRead === "session" && !isAuthenticated(c);
    app.use("*", async (c, next) => {
        ...
        if (!authToken)
            return next();
        if (path === "/guide" || path === "/setup" || path === "/agent-howto")
            return next();
    ...
    // --- pages and docs ---
    const withOrigin = (text, c) => text.replaceAll(LOCAL_ORIGIN, new URL(c.req.url).origin);
    const injectHead = (text, head) => {
        const headClose = text.lastIndexOf("</head>");
        return headClose >= 0
            ? `${text.slice(0, headClose)}${head}${text.slice(headClose)}`
            : `${head}${text}`;
    };
    const withDocumentTitle = (text, title) => {
        if (!title)
            return text;
        ...
        if (titleStart < 0)
            return injectHead(text, titleTag);
    ...
    const postPreviewHead = (post, request) => {
    ...
    const configuredViewerHtml = (c, opts = {}) => {
        const pageTitle = opts.post?.title ?? opts.title;
        const html = withDocumentTitle(withViewerConfig(withOrigin(viewerHtml, { req: { url: c.req.url } }), c.req.raw, !!publicRead && !isAuthenticated(c), pageTitle), pageTitle);
    ...
    app.delete("/api/sessions/:id", async (c) => {
        ...
        if (!(await store.removeSession(id)))
            return c.json({ error: "session not found" }, 404);
        bus.broadcast({ type: "session-deleted", id });
        return c.json({ ok: true });
    });
    const listSessionPosts = async (c) => {
        const session = await store.getSession(c.req.param("id"));
        if (!session)
            return c.json({ error: "session not found" }, 404);
        const posts = await store.listPosts(session.id);
        return c.json(c.req.query("hydrate") === "1"
    ...
    async function publish(c, body, surfaces) {
    ...
    app.patch("/api/posts/:id", async (c) => {
        ...
        if (content !== undefined) {
            ...
            const targetIdx = surface !== undefined
            ...
            if (targetIdx < 0) {
                ...
                return c.json({
                    error: 'content update requires a "surface" target (id or index) for multi-surface posts',
                }, 400);
            }
    ...
    app.post("/api/posts/:id/surfaces", async (c) => {
        const body = await c.req.json().catch(() => null);
    ...
    app.patch("/api/posts/:id/surfaces/:target", async (c) => {
        const body = await c.req.json().catch(() => null);
    ...
    app.delete("/api/posts/:id/surfaces/:target", async (c) => {
    ...
    app.patch("/api/posts/:id/surfaces", async (c) => {
        const body = await c.req.json().catch(() => null);
    ...
    app.post("/api/comments", async (c) => {
        ...
        if ("error" in result)
            return c.json({ error: result.error }, result.status);
        return c.json({ ...result.comment, ...(result.userFeedback && { userFeedback: result.userFeedback }) }, 201);
    });
    ...
    app.get("/api/version", async (c) => {
        if (!version)
            return c.json({ current: null, latest: null, updateAvailable: false });
    ...
    app.get("/api/comments", async (c) => {
        ...
        if (isUnauthenticatedSessionRead(c)) {
            if (!sessionId && !surfaceId)
                return c.json({ error: "session or surface required" }, 401);
            if (sessionId && !(await store.getSession(sessionId))) {
                return c.json({ error: "session not found" }, 404);
            }
            if (surfaceId) {
        ...
        if (waitSeconds > 0) {
            if (!acquireHold())
                return c.json({ error: "too many concurrent connections" }, 503);
    ...
    const renderPostPage = async (c) => {
        ...
        if (surfaceParam == null)
            return c.html(configuredViewerHtml(c, { post }));
        ...
        if (ver && Number(ver) !== post.version) {
            ...
            if (!old)
                return c.text(`Version ${ver} not available`, 404);
            title = old.title;
        ...
        const doc = await cachedRender(cacheKey, async () => {
            ...
            const rendered = surface.kind === "markdown"
                ...
                : surface.kind === "code"
                    ...
                    : surface.kind === "terminal"
                        ...
                        : await renderDiff(surface, { theme: themeId, mode }).catch((e) => ({
                            body: `<div class="rich-error">Couldn’t render diff — ${escapeHtml(e instanceof Error ? e.message : "render error")}</div>`,
        ...
        try {
            body = envelope
                ? {
                    ...
                    contentType: typeof envelope.contentType === "string"
